...
namespace DistributedGlobalPathPlanner
{
    PhaseSynchronizer::PhaseSynchronizer()
        : m_phase(Common::PHASE::PHASE_1), m_transitingPhase(true), m_handlerPtr(std::make_shared<DistributedGlobalPathPlannerHandler>())

          ,
          m_dimension(Common::DIMENSION::DIM_2), m_expiryDurationMicroSec(15 * 1000000), m_pointRemovalRadius(0.45), m_agentRadius(0.3), m_waypointReachedBoundary(1.5), m_desiredHeight(2.0), m_pathPlanningParams()
    {
    ...
    PhaseSynchronizer::PhaseSynchronizer(const Common::DistributedGlobalPathParams &distributedGlobalPathParams,
        ...
        : m_phase(Common::PHASE::PHASE_1), m_transitingPhase(true), m_handlerPtr(std::make_shared<DistributedGlobalPathPlannerHandler>())
          ...
          ,
          m_dimension(distributedGlobalPathParams.dimension), m_expiryDurationMicroSec(distributedGlobalPathParams.expiryDurationMicroSec), m_pointRemovalRadius(distributedGlobalPathParams.pointRemovalRadius), m_agentRadius(distributedGlobalPathParams.agentRadius), m_waypointReachedBoundary(distributedGlobalPathParams.waypointReachedBoundary), m_desiredHeight(distributedGlobalPathParams.desiredHeight), m_pathPlanningParams(pathPlanningParams)
    {
        if (m_dimension == Common::DIMENSION::DIM_2)
        {
            m_pathPlanning2DHandle.setParams(m_pathPlanningParams);
    ...
    void PhaseSynchronizer::UpdateGoTherePath()
    {
        ...
        else
        {
            ...
            {
                if (goTherePath.at(i).position != m_goTherePath.at(i).position ||
                    goTherePath.at(i).headingRad != m_goTherePath.at(i).headingRad)
                {
                    resetPath = true;
                    break;
                }
        ...
        if (resetPath)
        {
            ...
            std::cout << "Agent" << m_ownAgentID << ": reset as path has changed" << std::endl;
    ...
    void PhaseSynchronizer::UpdatePhasesOfAgentsInTeam()
    {
        ...
        {
            std::cout << "Agent" << m_ownAgentID << ": reset as phases are not in sync" << std::endl;
    ...
    void PhaseSynchronizer::DoPhase2()
    {
        ...
        else
        {
            ...
            if (agentsGoTherePathAndWaypointProgressCopy.size() == m_phasesOfAgentsInTeam.size())
            {
                ...
                if (!pathIsSimilar)
                {
                    std::cout << "Agent" << m_ownAgentID << ": best path from agents are not in consensus" << std::endl;
    ...
    void PhaseSynchronizer::OnEnterPhase3()
    {
        ...
        {
            std::cout << "Agent" << m_ownAgentID << ": global path is empty" << std::endl;
        ...
        {
            std::cout << "Agent" << m_ownAgentID << ": unable to get both lidar point cloud" << std::endl;
                ...
                for (auto &&processedPathOfAgents : agentsProcessedPathOfAgentsCopy)
                {
                    ...
                    if (processedPathOfAgents.second.size() == m_ownProcessedPathOfAgents.size())
                    {
                        for (auto &&processedPath : processedPathOfAgents.second)
                        {
                            ...
                            {
                                std::cout << "Agent" << m_ownAgentID << ": unable to match agents id" << std::endl;
            ...
            if (agentsBestProcessedPathCopy.size() == m_phasesOfAgentsInTeam.size())
            {
                ...
                for (auto &&agentPath1 : agentsBestProcessedPathCopy)
                {
                    ...
                    for (auto &&agentPath2 : agentsBestProcessedPathCopy)
                    {
                        ...
                        {
                            if (agentPath1.second.at(i) != agentPath2.second.at(i))
                            {
                                pathIsSimilar = false;
                                break;
                            }
