...
namespace Formation2DWithYaw
{
                                                      ...
                                                      Eigen::Matrix<double, Eigen::Dynamic, 1>& positions2DInFormation) const
    {
        ...
        positions2DInFormation(0,0) = m_desiredX + d_x + (l/2) * std::sin(m_desiredYaw + d_yaw);
        positions2DInFormation(1,0) = m_desiredY + d_y + (l/2) * std::cos(m_desiredYaw + d_yaw);
        positions2DInFormation(2,0) = m_desiredX + d_x - (l/2) * std::sin(m_desiredYaw + d_yaw);
        positions2DInFormation(3,0) = m_desiredY + d_y - (l/2) * std::cos(m_desiredYaw + d_yaw);
    }
    ...
    {
        positions2DJacobian.resize(4,4);
        ...
        positions2DJacobian(0,2) = (l/2) * std::cos(m_desiredYaw + d_yaw);
        positions2DJacobian(0,3) = 0.5 * m_desiredDistance * std::sin(m_desiredYaw + d_yaw);

        //second row pt1y
        positions2DJacobian(1,0) = 0;
        positions2DJacobian(1,1) = 1;
        positions2DJacobian(1,2) = -(l/2) * std::sin(m_desiredYaw + d_yaw);
        positions2DJacobian(1,3) = 0.5 * m_desiredDistance * std::cos(m_desiredYaw + d_yaw);

        //third row pt2x
        positions2DJacobian(2,0) = 1;
        positions2DJacobian(2,1) = 0;
        positions2DJacobian(2,2) = -(l/2) * std::cos(m_desiredYaw + d_yaw);
        positions2DJacobian(2,3) = -0.5 * m_desiredDistance * std::sin(m_desiredYaw + d_yaw);
